 ...
 *
 *
 * This app was created  by Truman Marshall, Ryan Choi, and Tanay Srivastava. Our app is PetBnB! An AirBnB for pets, where users can
 * login, search for users, input payment, and search locations through google maps. Our Login form allows the user to place a
 * username and password and enter our app. Payment, done by Ryan Choi, asks for basic information in a list of textfields. This information
 * will be potentially stored and remembered in the SQL Database. The whole login form, which remembers entered usernames and passwords, was also completed by Ryan Choi.
 * Search Users, done by Truman Marshall, connects to the SQL Database, pulls the
 * list of names and information associated with the users, and displays it in a nice alphabetical layout. A search feature allows for the app user to search
 * for whichever name desired, and remembers the previous search if the user accidently clicks out of the tab. The hamburger sidemenu layout,
 * completed by Truman, allows easy access to various tabs in the app. Google Maps, done by Tanay Srivastava, allows for a user to experience
 * built-in Native Maps with Satellite and Street mode. A user can
 * search for any location in the world, add the pin to the maps interface, and move towards the entered location. A user can have a street-view of the location,
 * and interact with the native google maps user function such as getting directions, information, etc.
 *
 * Written By: Tanay Srivastava
 *
 */
